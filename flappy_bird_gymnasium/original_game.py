...
ASSETS_DIR = "./flappy_bird_gymnasium/assets"
...
def main():
    ...
    IMAGES["gameover"] = pygame.image.load(
        ASSETS_DIR + "/sprites/gameover.png"
    ).convert_alpha()
    # message sprite for welcome screen
    IMAGES["message"] = pygame.image.load(
        ASSETS_DIR + "/sprites/message.png"
    ).convert_alpha()
    # base (ground) sprite
...
def main_game(movement_info):
    ...
    while True:
        ...
        if (
            len(upper_pipes) > 0
            and upper_pipes[0]["x"] < -IMAGES["pipe"][0].get_width()
        ):
            upper_pipes.pop(0)
            lower_pipes.pop(0)
...
def check_crash(player, upper_pipes, lower_pipes):
    ...
    if player["y"] + player["h"] >= BASE_Y - 1:
        ...
        for up_pipe, low_pipe in zip(upper_pipes, lower_pipes):
            ...
            low_collide = pixel_collision(
                player_rect, lPipeRect, p_hitmask, low_hitmask
            )
